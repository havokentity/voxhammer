...
void RegisterCoreCvars() {
    ...
    reg("renderer.gi.intensity", "1.0", "QUALITY indirect-GI strength: scales the sky-fill + surface inter-reflection bounce. >1 brightens dark enclosed rooms (skylight barely reaches deep interiors); 1 = physical. Artistic dial.", {.type = CVarType::Float, .flags = CVAR_ARCHIVE, .range_min = 0.0f, .range_max = 8.0f, .range_step = 0.25f});
